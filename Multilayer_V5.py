...
class StartGUI(tk.Tk):
	...
	def isQuincux(self, runObject, sample):
		print("### - Quincux Determination")
		dicGexel=runObject.dicSample[sample.name].dicGexelRaw
		listCoordinate=[]
		for coordinate in dicGexel.keys():
			x = int(coordinate.split('x')[0])
			y = int(coordinate.split('x')[1])
			listCoordinate.append([x, y])
		minGexel=min(listCoordinate, key=lambda x: (x[0], x[1]))
		if f"{minGexel[0]+1}x{minGexel[1]}" in dicGexel.keys() or f"{minGexel[0]}x{minGexel[1]+1}" in dicGexel.keys():
			print("Quincux = False")
			return False, None
		print(f"Quincux = True : {minGexel[0]}x{minGexel[1]}")
		return True, minGexel

	
	def changeCoordinatesForAgglomeration(self, runObject, minGexel):
		test=[[[x,y] for x in range(1,runObject.calculatedX+1)] for y in range(1,runObject.calculatedY+1)]
		tempList = []
		#Parité
		if minGexel[0]%2 == minGexel[1]%2:
			start = 0
		else :
			start = 1
		for y in range(start,len(test),2):
			diag=[]
			x = 0
			while y >= 0 and x<len(test[y]):
				diag.append(test[y][x])
				y -= 1
				x += 1
			tempList.append(diag)
		if len(test)%2 == start%2:
			start = 1
		else :
			start = 2
		for x in range(start,len(test[0]),2):
			diag=[]
			y = len(test)-1
			while y >= 0 and x<len(test[y]):
				diag.append(test[y][x]) #y x?
				y -= 1
				x += 1
			tempList.append(diag)
		
		#Plus grande taille
		maxLenLine = max(len(line) for line in tempList)
		for index, line in enumerate(tempList):
			if len(line) == maxLenLine:
				indexFirstMaxLenLine = index
				break
		for j in range(indexFirstMaxLenLine,len(tempList)-1):
			if len(tempList[j+1]) != maxLenLine:
				indexLastMaxLenLine = j
				break

		if indexFirstMaxLenLine != indexLastMaxLenLine and indexFirstMaxLenLine != indexLastMaxLenLine + 1:
			if tempList[indexLastMaxLenLine][0][0] == 1:
				for j in range(indexLastMaxLenLine, indexFirstMaxLenLine, -1):
					i = 0
					while tempList[j-1][i][0]+1 != tempList[j][i][0] or tempList[j-1][i][1]+1 != tempList[j][i][1]:
						tempList[j-1].insert(0,[0,0])
						i+=1
				maxLenLine = len(tempList[indexFirstMaxLenLine])
			else :
				for j in range(indexFirstMaxLenLine, indexLastMaxLenLine):
					i = 0
					while tempList[j+1][i][0]-1 != tempList[j][i][0] or tempList[j+1][i][1]-1 != tempList[j][i][1]:
						tempList[j+1].insert(0,[0,0])
						i+=1
				maxLenLine = len(tempList[indexLastMaxLenLine])
			for j in range(indexFirstMaxLenLine,indexLastMaxLenLine+1):
					while len(tempList[j]) < maxLenLine:
						tempList[j].append([0,0])		
		
		for j in range(indexFirstMaxLenLine, 0, -1):
			i = 0
			while tempList[j-1][i][0]+1 != tempList[j][i][0] or tempList[j-1][i][1]+1 != tempList[j][i][1]:
				tempList[j-1].insert(0,[0,0])
				i+=1
			while len(tempList[j-1]) < maxLenLine:
				tempList[j-1].append([0,0])
		for j in range(indexLastMaxLenLine, len(tempList)-1):
			i = 0
			while tempList[j+1][i][0]-1 != tempList[j][i][0] or tempList[j+1][i][1]-1 != tempList[j][i][1]:
				tempList[j+1].insert(0,[0,0])
				i+=1
		...
		dictAssCoor={} # bases : nouvelles (commencent à 0)
		for j in range(0,len(tempList)):
			for i in range(0,len(tempList[j])):
				if tempList[j][i] != [0,0]:
					dictAssCoor[f"{tempList[j][i][0]}x{tempList[j][i][1]}"]=[i,j]
		runObject.dictCoor = dictAssCoor
	
	@staticmethod
	def associateCoordinate(runObject, old):
		dictCoor = runObject.dictCoor
		new = []
		for coordinate in old:
			new.append(dictCoor[f"{coordinate[0]}x{coordinate[1]}"])
		return np.asarray(new)
	
	@staticmethod
	def returnCoordinate(x, y, runObject):
		dictCoor = runObject.dictCoor
		key_list = list(dictCoor.keys())
		val_list = list(dictCoor.values())
		position = val_list.index([x,y])
		coordinate = key_list[position]
		x = int(coordinate.split('x')[0])
		y = int(coordinate.split('x')[1])
		return x, y
	...
	def create_matrix(self, sampleData, runObject):
		...
		runObject.quincux, minGexel=self.isQuincux(runObject, sample)
		if runObject.quincux:
			self.changeCoordinatesForAgglomeration(runObject, minGexel)
	...
	def agglomerative(self, dicGeneSample, sample, runObject, indexMinPattern):
		...
		for geneSample in dicGeneSample.keys():
			...
			if len(tempListCoordinate) >= runObject.minPattern:
				clustering = AgglomerativeClustering(n_clusters=None, metric='euclidean', #deprecated
					linkage='single', distance_threshold=1.5)
				if runObject.quincux: #Il y a un print bizarre quand on clique sur un cluster
					tempListCoordinate = self.associateCoordinate(runObject, tempListCoordinate) #pas opti : il faut le faire en amont
				labels = clustering.fit_predict(tempListCoordinate)	#On met nos coordonnées correspondantes
				...
				for i in frequencies:
					...
					if count >= int(indexMinPattern):
						...
						for o in tempListCoordinate[labels==index]:
							if runObject.quincux:
								o[0], o[1]=self.returnCoordinate(o[0], o[1], runObject)
							listTemp.append(f'{o[0]}x{o[1]}') #INDEX -> on renvoit les indexs de base, à l'aide de notre dictionnaire
...
class InputGUIBatch(tk.Frame):
	...
	def __init__(self, parent, controller, runObject):
		...
		self.labelRound = tk.Label(self.frameInformation, text=': Round coordinates')
...
class HeatmapGOAll(object):
	...
	def __init__(self, master, listGeneQuery, table, titleGO, comboGOBank, minV, maxV, annotation):
		...
		if annotation == True:
			plot = sns.heatmap(table.transpose(), norm=LogNorm(), linewidths=0.3, cmap=sns.light_palette(color='green', as_cmap = True, reverse = True),
...
class HeatmapGO(object):
	...
	def __init__(self, master, listGeneQuery, table, titleGO, comboGOBank):
		...
		if df2.shape[0] >= 40:
			...
			df3 = df2
		plot = sns.heatmap(df3, cmap=cmap, linewidths=0.3,
...
class Barplot(object):
	...
	def __init__(self, master, df, titleGO, comboGOBank):
		...
		self.fig = Figure()
		ax = self.fig.add_subplot(111)
		plot = sns.barplot(x=1/df2['p-values'] , y=[i for i in range(1, len(df2.index)+1)],
...
class mainGUI(object):
	...
	def __init__(self, master, runObject):
		...
		if self.runObject.mode == 'cluster':
			...
			self.umapButton = ttk.Button(self.frame, text = 'UMAP',
				command = lambda : self.wrapperUMAP())
	...
	def wrapperUMAP(self):
		self.umapWrapper = tk.Toplevel(self.master)
		self.umapArgs(self.umapWrapper)
		self.umapWrapper.title('Arguments for UMAP')
		self.umapWrapper.minsize(280, 70)

	def umapArgs(self, master):
		self.frameArg = tk.Frame(master)
		self.listMetrics=["euclidean", "manhattan" ,"chebyshev" ,"minkowski" ,"canberra", "braycurtis" ,"haversine",
					 "cosine", "correlation"]
		n_clusters = IntVar(value=3)
		n_pcs = IntVar(value=15)
		n_neighbors = IntVar(value=15)
		min_dist = DoubleVar(value=0.1)
		resolution= DoubleVar(value=0.5)
		norm = BooleanVar(value=False)
		labelN_cluster = Label (self.frameArg, text = "Number of pcs :")
		entryN_cluster = Entry(self.frameArg, textvar = n_pcs, width=5)
		labelN_neighbors = Label (self.frameArg, text = "Number of neighbors :")
		entryN_neighbors = Entry(self.frameArg, textvar = n_neighbors, width=5)
		labelMin_dist = Label (self.frameArg, text = "Min dist :")
		entryMin_dist = Entry(self.frameArg, textvar = min_dist, width=5)
		labelMetrics = Label (self.frameArg, text = "Metric:")
		resolutionEntry = Entry(self.frameArg, textvar = resolution, width=5)
		metrics = ttk.Combobox(self.frameArg, values=self.listMetrics)
		metrics["state"]="readonly"
		metrics.current(0)
		normBox=tk.Checkbutton(self.frameArg, variable = norm, onvalue = True, offvalue = False, text="Re-normalize", font="Arial 11")
		#frameThresoldDiff = Frame(master)
		#searchButton = ttk.Button(frameThresoldDiff, text = 'Search', command = lambda : self.matrixForGene(self.colorList))
		#searchButton.pack(side = TOP, fill=X)
		#reset = ttk.Button(frameThresoldDiff, text = 'Reset', command = lambda : self.resetMatrix())
		#reset.pack(side = TOP, fill=X)
		labelN_cluster.pack(side = LEFT)
		entryN_cluster.pack(side=LEFT)
		labelN_neighbors.pack(side=LEFT)
		entryN_neighbors.pack(side=LEFT)
		labelMin_dist.pack(side=LEFT)
		entryMin_dist.pack(side=LEFT)
		Label(self.frameArg, text='Resolution :').pack(side=LEFT)
		resolutionEntry.pack(side=LEFT)
		metrics.pack(side=BOTTOM)
		labelMetrics.pack(side=BOTTOM)
		normBox.pack(side=BOTTOM)
		self.frameArg2= tk.Frame(master)
		labelThresholdDiff = Label(self.frameArg2, text='Threshold expression :')
		labelThresholdDiff.pack(side = TOP, fill=X)
		#frameThresoldminDiff = Frame(self.frameArg2)
		#labelThresholdDownDiff = Label(self.frameArg2, text='Down :')
		#labelThresholdDownDiff.pack(side = LEFT)
		#minTresholdEntryDiff = Entry(self.frameArg2, textvar=self.thresholdDownDiff, width=5)
		#minTresholdEntryDiff.pack(side = LEFT)
		#frameThresoldminDiff.pack(side=LEFT)
		frameThresoldmaxDiff = Frame(self.frameArg2)
		labelThresholdUpDiff = Label(self.frameArg2, text='Up :')	
		labelThresholdUpDiff.pack(side = LEFT)
		thresholdUpDiff = DoubleVar()
		maxTresholdEntryDiff = Entry(self.frameArg2, textvar=thresholdUpDiff, width=5)
		maxTresholdEntryDiff.pack(side = LEFT)
		frameThresoldmaxDiff.pack(side=RIGHT)
		#frameThresoldDiff.pack(side = BOTTOM)
		self.runumapButton = ttk.Button(self.frameArg2, text='Run UMAP',
			command = lambda : self.umapWindow('Counts Matrix', self.runObject.dicSample[self.nameSample].dicGexelRaw,
				self.runObject.dicSample[self.nameSample].dicSampleRaw, self.runObject.dicSample[self.nameSample].maxCountRaw,
				self.runObject.dicSample[self.nameSample].minCountRaw, 'rainbow', self.countDisplayString, self.nameSample,
				n_clusters.get(), n_neighbors.get(), min_dist.get(), metrics.get(), thresholdUpDiff.get(), n_pcs.get(),
				resolution.get(), norm.get()))
		self.runumapButton.pack(side=BOTTOM)
		self.frameArg.pack(side=TOP)
		self.frameArg2.pack(side=BOTTOM)


		#self.matrixWindow('Gene Expressed Matrix', self.runObject.dicSample[self.nameSample].listGexelDiff,
					#self.runObject.dicSample[self.nameSample].dicSampleDiff, 4, -4, 'diffGeneGradient', self.countDisplayStringDiff, self.nameSample, []))
		#listGexelDiff -> nom (coordonnées) des Gexels ayant des gènes differement exprimées (suivant les tresholds)
		#dictSampleDiff -> clef gènes, valeurs object GeneSample, GeneSample.dic_coordinate_count contient l'expression diff
		#diffGeneGradient ?
		#countDisplayStringDiff -> l'expression différentielle affichée
		#master.destroy()

		#self.dicGeneSample[geneName].dic_coordinate_count[coordinateTemp] >= float(self.thresholdUp)
		#self.dicGeneSample[geneName].dic_coordinate_count[coordinateTemp]

	
	def umapWindow(self, title, gexel, geneSample, maxCount, minCount, gradient, stringDisplay, nameSample, n_clusters, n_neighbors, min_dist, metric, thresholdUpDiff, n_pcs, resolution, norm):
		self.windowGUI = tk.Toplevel(self.master)
		self.title = f'UMAP All - {title}'
		umapGUI(self.title, self.windowGUI, self, n_pcs, n_neighbors, min_dist, metric, thresholdUpDiff, resolution, norm)
		self.windowGUI.title(self.title)
		self.umapWrapper.destroy()
	...
	def wrapperMinPattern(self, master):
		if self.thresholdMinPattern.get() != self.runObject.minPattern:
			...
			StartGUI.generateMatrixSimilarity(StartGUI,self.runObject.dicSample[self.nameSample].dicSampleDiff,
				self.runObject, 'Data/Similarity_Matrix_Sample', self.runObject.dicSample[self.nameSample].name)
		master.destroy()
...
class resultGOClass(object):
	...
	def heatmapClass(self, master):
		...
		ax = self.fig.add_subplot(111)
		cmap = LinearSegmentedColormap.from_list('Custom', ['#D3D3D3', '#319819'], 2)
		plot = sns.heatmap(self.df, cmap=cmap, linewidths=0.3,
			linecolor='black', ax=ax, xticklabels=True, yticklabels=True)
...
class wrapperClass(object):
			...
			if int(self.dicClass[name]) == self.runObject.minPattern:
				...
				for geneSample in dic_geneSampleDiffCopy.keys():
					...
					if len(tempListCoordinateCopy) >= int(self.dicClass[name]):
						clustering_temp = AgglomerativeClustering(n_clusters=None, metric='euclidean',
							linkage='single', distance_threshold=1.5)
	...
	def heatmapClass(self, master):
		...
		ax = self.fig.add_subplot(111)
		cmap = LinearSegmentedColormap.from_list('Custom', ['#D3D3D3', '#319819'], 2)
		plot = sns.heatmap(self.dfTT, cmap=cmap, linewidths=0.3,
			linecolor='black', ax=ax, xticklabels=True, yticklabels=True)
...
class matrixGUI(object):
	...
	def __init__(self, title, master, runObject, countDisplayStringCoor, countDisplayString, nameSample, dicGexel, dicGeneSample, maxCountGexel, minCountGexel, gradient, listGOBank, currentLouvain):
		...
		if 'All - Communities' in title:
			...
			self.title = title
		self.indexLog = 0
		self.multSize=self.quincuxModSize()
		...
		self.can = Canvas(self.frameMatrix, width=((self.runObject.sizeX+1)*self.indexMiniSizeGexel)+1+6, 
			height = ((self.runObject.sizeY+1)*self.indexMiniSizeGexel)+1+6)
		self.can.configure(background='darkgrey') #Background
		...
		self.varGradient = StringVar()
		self.varGradient.set(self.getLegendGradient())
		self.legendGradient = Label(self.colorGradientFrame, textvariable=self.varGradient)
		...
		if self.gradient == 'patternGradient' or self.gradient == 'communitiesGradient':
			self.colorList = []
		elif self.gradient == 'umap':
			self.colorList = umapGUI.setColorUmap(self.runObject.dictClusterGexel)
		else:
		...
		if self.gradient != 'communitiesGradient':
			...
			if self.gradient == 'patternGradient' or self.gradient == 'umap':
	...
	def quincuxModSize(self):
		if self.runObject.quincux:
			return math.sqrt(2)
		return 1
		
	def getLegendGradient(self):
		legend = ""
		self.indexLog == 1
		if "Counts" in self.title and "Normalized" not in self.title:
			legend = "Raw Counts"
		elif "Normalized" in self.title:
			legend = "Normalized Counts"
		elif "Gene Expressed" in self.title:
			legend = "Expression Level"
		else:
			legend = ""
		if self.indexLog == 1:
			legend += " (log2)"
	...
	def saveCanvas(self):
		filename = asksaveasfilename(defaultextension=".png", filetypes=(('Portable Network Graphics', '*.png'),))
		if filename != '':
			if self.gradient != 'patternGradient' and self.gradient != 'communitiesGradient' and self.gradient != 'umap':
				self.colorGradient()
	...
	def menu(self):
		...
		if self.gradient == 'communitiesGradient':
			...
			if self.gradient == 'patternGradient':
				...
				self.listGeneGO = []
				self.frameInfoPattern = tk.Frame(self.frameSearchButton)
				self.infoText = Label(self.frameInfoPattern, text='Patterns Informations :\nCluster | Number of gexel')
				self.infoText.pack(fill=X)
				self.listBoxPatterns = Listbox(self.frameInfoPattern, height=10, width=15)
				umapGUI.fillListboxUmap(self.runObject, self.listBoxPatterns)
				self.listBoxPatterns.bind('<Double-Button>', lambda event, listBox = self.listBoxPatterns, index = 2 : self.selection_gene(event, listBox, index))
				self.listBoxPatterns.bind('<Return>', lambda event, listBox = self.listBoxPatterns, index = 2 : self.selection_gene(event, listBox, index))
				self.listBoxPatterns.pack(fill=X)
				self.GOButton = ttk.Button(self.frameInfoPattern, text = 'Gene Ontology', command = lambda : self.geneOntology(self.listGeneGO, self.listGOBank, self.temp_title))
				self.GOButton.pack(side=BOTTOM, fill = X, pady=10)
				...
				self.labelThresholdMinMax = Label(self.frameThresold, text='Expression level range :')
				...
				if self.gradient == 'diffGeneGradient':
					...
					self.labelLog = tk.Label(self.frameLogValue, text=': Log2 values.')
	...
	def selection_gene(self, event, listBox, index):
		...
		if index == 0:
			...
			self.geneSelected = listBox.get(ACTIVE).split(' | ')[0]
			self.clusterMatrix()
	...
	def colorGradient(self):
		...
		for x in range(0, 256):
			...
			self.draw.ellipse([(x*2)+(3*self.indexMiniSizeGexel), 0, (x*2)+(3*self.indexMiniSizeGexel) + 2, 16], fill = self.rgb(r, g, b), outline = self.rgb(r, g, b))
	...
	def colors(self):
		...
		for color in list_color_hex:
			...
			self.draw.ellipse([x1+(3*self.indexMiniSizeGexel), 0, x2+(3*self.indexMiniSizeGexel), 16], fill = color, outline = color)
			x1 += 50
			x2 += 50
	...
	def similarity(self):
		...
		if self.patternDisplayString.get() == '':
			...
			x1 = 1
			x2 = self.indexMiniSizeGexel*self.multSize
			y1 = 1
			y2 = self.indexMiniSizeGexel*self.multSize
				...
				for x in range(1, self.runObject.sizeX+1):
					...
					if coordinateTemp in self.dicGexel:
						if coordinateTemp in finalDic.keys():
							can_temp = self.can.create_oval(x1+self.indexMiniSizeGexel, y1+self.indexMiniSizeGexel, x2+self.indexMiniSizeGexel, y2+self.indexMiniSizeGexel,
								fill = self.colorList[finalDic[coordinateTemp]], activefill = 'white', activeoutline = 'white')
							self.draw.ellipse([x1+self.indexMiniSizeGexel, y1+self.indexMiniSizeGexel, x2+self.indexMiniSizeGexel, y2+self.indexMiniSizeGexel],
					...
					"""		
						...
						self.can.create_oval(x1+self.indexMiniSizeGexel, y1+self.indexMiniSizeGexel, x2+self.indexMiniSizeGexel, y2+self.indexMiniSizeGexel,
							fill = 'white')
						self.draw.ellipse([x1+self.indexMiniSizeGexel, y1+self.indexMiniSizeGexel, x2+self.indexMiniSizeGexel, y2+self.indexMiniSizeGexel],
	...
	def drawingMatrix(self, colorList):
		...
		y2 = self.indexMiniSizeGexel*self.multSize
		self.varGradient.set(self.getLegendGradient())
		if self.gradient != 'patternGradient' and self.gradient != 'umap':
			if self.indexLog == 1:
				...
				if self.thresholdMin.get() == '':
					...
					self.minValueGradient.set(float(self.thresholdMin.get()))
		if self.gradient == 'umap': #TODO: à changer
			dictGexelCluster = { v: k for k, l in self.runObject.dictClusterGexel.items() for v in l }
		for y in range(1, self.runObject.sizeY+1):
			x1 = 1
			x2 = self.indexMiniSizeGexel*self.multSize
			for x in range(1, self.runObject.sizeX+1):
				...
				if self.gradient == 'diffGeneGradient' or self.gradient == 'patternGradient':
					if coordinateTemp in self.dicGexel:
						can_temp = self.can.create_oval(x1+(self.indexMiniSizeGexel), y1+(self.indexMiniSizeGexel),
									  x2+(self.indexMiniSizeGexel), y2+(self.indexMiniSizeGexel),
									  fill = 'lightgrey')
						self.draw.ellipse([x1+(self.indexMiniSizeGexel), y1+(self.indexMiniSizeGexel),
						 x2+(self.indexMiniSizeGexel), y2+(self.indexMiniSizeGexel)],
						 fill = 'lightgrey', outline = 'black')
						#can_temp = self.can.create_oval(x1+self.indexMiniSizeGexel, y1+self.indexMiniSizeGexel, x2+self.indexMiniSizeGexel, y2+self.indexMiniSizeGexel,
						#	fill = 'lightgrey')
					...
					if coordinateTemp in self.dicGexel:
						can_temp = self.can.create_oval(x1+(self.indexMiniSizeGexel), y1+(self.indexMiniSizeGexel),
									  x2+(self.indexMiniSizeGexel), y2+(self.indexMiniSizeGexel),
									  fill = self.colorList[dictGexelCluster[coordinateTemp]]) #colorList = dictClusterColor
						self.draw.ellipse([x1+(self.indexMiniSizeGexel), y1+(self.indexMiniSizeGexel),
						x2+(self.indexMiniSizeGexel), y2+(self.indexMiniSizeGexel)],
						fill = 'lightgrey', outline = 'black')
	...
	def clusterMatrix(self):
		"""
		Display the selected cluster's pattern
		"""
		self.saveCanvasImage = PIL.Image.new('RGBA', (int(((self.runObject.sizeX+1)*(self.indexMiniSizeGexel))+16),
			(int((self.runObject.sizeY+1)*(self.indexMiniSizeGexel))+16)), (0,0,0,0))
		self.draw = PIL.ImageDraw.Draw(self.saveCanvasImage)
		...
		self.maxValueGradient.set('')
		self.minValueGradient.set('')
		clusterName = self.geneSelected
		indexGexelCreate = 0
		#self.patternDisplayString.set('')
		self.can.delete('all')
		self.master.title(f'{self.title} - {clusterName}')
		self.temp_title = f'{self.title} - {clusterName}'
		listColor = ['green', 'blue', 'red', 'gold', 'purple',
			'pink', 'orange', 'yellow', 'darkgreen', 'darkred', 
			'darkblue', 'limegreen', 'darkorange', 'deeppink', 'cyan']
		x1 = 1
		...
		y1 = 1
		y2 = self.indexMiniSizeGexel*self.multSize
		all_gexel=[] #TODO à changer ; dicGexel ?
		for key in self.runObject.dictClusterGexel.keys():
			all_gexel+=self.runObject.dictClusterGexel[key]
		for y in range(1, self.runObject.sizeY+1):
			x1 = 1
			x2 = self.indexMiniSizeGexel*self.multSize
			for x in range(1, self.runObject.sizeX+1):
				...
				indexFind = 0
				#print(index)
				if coordinateTemp in self.runObject.dictClusterGexel[clusterName]:
					can_temp = self.can.create_oval(x1+self.indexMiniSizeGexel, y1+self.indexMiniSizeGexel, x2+self.indexMiniSizeGexel, y2+self.indexMiniSizeGexel,
						fill = self.colorList[clusterName], activefill = 'white', activeoutline = 'white')
					self.draw.ellipse([x1+self.indexMiniSizeGexel, y1+self.indexMiniSizeGexel, x2+self.indexMiniSizeGexel, y2+self.indexMiniSizeGexel],
						fill = self.colorList[clusterName], outline = 'black')
					...
					indexFind += 1
					indexGexelCreate += 1
				if indexFind == 0 and (coordinateTemp in all_gexel):
					can_temp = self.can.create_oval(x1+self.indexMiniSizeGexel, y1+self.indexMiniSizeGexel, x2+self.indexMiniSizeGexel, y2+self.indexMiniSizeGexel,
						fill = 'lightgrey')
					self.draw.ellipse([x1+self.indexMiniSizeGexel, y1+self.indexMiniSizeGexel, x2+self.indexMiniSizeGexel, y2+self.indexMiniSizeGexel],
						fill = 'lightgrey', outline = 'black')
...
class umapGUI(object):
	...
	def __init__(self, title, master, mainGUI, n_pcs, n_neighbors, min_dist, metric, thresholdUpDiff, resolution, norm):
		self.mainGUI = mainGUI
		self.master = master
		self.runObject = mainGUI.runObject
		self.nameSample = mainGUI.nameSample
		self.n_neighbors = n_neighbors
		self.n_clusters = n_pcs
		self.n_pcs = n_pcs
		self.min_dist = min_dist
		self.metric = metric
		self.thresholdUpDiff = thresholdUpDiff
		self.resolution = resolution
		self.norm = norm

		fig = self.drawingUmap()
		self.frame = Frame(self.master)
		self.figureCanvas = FigureCanvasTkAgg(fig, master = self.frame)
		#self.toolbar = NavigationToolbar2Tk(self.figureCanvas, self.master)
		self.menuLabel = Label(self.frame, text = "Clusters :")
		self.clusterListBox = ttk.Combobox(self.master, values=[str(x) for x in range(0,self.n_clusters)])
		self.clusterListBox["state"]="readonly"
		self.patternButton = ttk.Button(self.frame, text = 'Spatial', width = 25, 
				command = lambda : self.wrapperPatterns())
		
		self.figureCanvas.draw()
		self.menuLabel.pack()
		#self.clusterListBox.pack()
		self.figureCanvas.get_tk_widget().pack()
		self.patternButton.pack()
		self.frame.pack()

	def drawingUmap(self):
		import scanpy as sc
		...
		if self.norm:
			...
			if self.runObject.norm[0]!='':
				df = pd.read_csv(self.runObject.norm[0], sep='\t', index_col=[0]).astype(float)
				df.index = df.index.str.upper()
				self.adata = sc.AnnData(df.T)
			else:
				self.adata = sc.AnnData(self.runObject.norm[1].T)
				print(self.runObject.norm[1].T)
		sc.pp.highly_variable_genes(self.adata, flavor = 'seurat', n_top_genes=2000)
		sc.tl.pca(self.adata, svd_solver='arpack')
		sc.pp.neighbors(self.adata, n_neighbors=self.n_neighbors, n_pcs=self.n_pcs, metric = self.metric, knn=True)
		sc.tl.umap(self.adata, min_dist=self.min_dist, neighbors_key='neighbors')
		sc.tl.louvain(self.adata, resolution=self.resolution, random_state=0, directed=True)
		dictClusterGexel = self.associateClusterGexel()
		mapper=self.adata.obsm['X_umap']
		#colors = [self.param.clusters[clustername].color for clustername in self.param.adata.obs['louvain'].tolist()]
		dictClusterColor = umapGUI.setColorUmap(dictClusterGexel.keys())
		colors = [dictClusterColor[cluster] for cluster in self.adata.obs['louvain'].tolist()]
		#colors = [int(cluster) for cluster in self.adata.obs['louvain'].tolist()]
		fig = Figure(figsize = (8, 8), dpi = 100)
		plot1 = fig.add_subplot(111)
		sc = plot1.scatter(mapper[:,0], mapper[:,1], c = colors, s=3)
		#plot1.legend(*sc.legend_elements(), title='clusters')
		markers = [plt.Line2D([0,0],[0,0],color=color, marker='o', linestyle='') for color in list(dictClusterColor.values())]
		plot1.legend(markers, dictClusterColor.keys(), numpoints=1)
		return fig
	
	def associateClusterGexel(self):
		dictClusterGexel={}
		for gexel in self.adata.obs_names:
			cluster = self.adata.obs.at[gexel,'louvain']
			if cluster in dictClusterGexel.keys():
				dictClusterGexel[cluster].append(gexel)
			else:
				dictClusterGexel[cluster]=[gexel]
		#self.runObject.clusterList = np.unique(self.adata.obs['louvain']).tolist()
		#dictClusterGexel = sorted(dictClusterGexel)
	...
	def wrapperPatterns2(self):
		...
		self.mainGUI.patternWindow.minsize(260, 60)

	def patternsArgs(self, master):
		self.mainGUI.frameArg = tk.Frame(master)
		self.mainGUI.runObject.minPattern
		self.mainGUI.thresholdMinPattern = IntVar()
		self.mainGUI.labelthresholdMinPattern = Label(self.mainGUI.frameArg, text='Min. Gexel(s) per pattern :')
		self.mainGUI.labelthresholdMinPattern.pack(side=LEFT)
		self.mainGUI.entryMinPattern = Entry(self.mainGUI.frameArg, textvar=self.mainGUI.thresholdMinPattern, width=3)
		self.mainGUI.entryMinPattern.pack(side=LEFT)
		self.mainGUI.thresholdMinPattern.set(int(self.runObject.minPattern))
		self.mainGUI.frameArg.pack(side=TOP)
		self.mainGUI.runcommunitiesButton = ttk.Button(master, text='Run Patterns',
			command = lambda : self.wrapperMinPattern(master))
		self.mainGUI.runcommunitiesButton.pack()

	def wrapperMinPattern(self, master):
		if self.mainGUI.thresholdMinPattern.get() != self.mainGUI.runObject.minPattern:
			self.runObject.minPattern = self.mainGUI.thresholdMinPattern.get()
			print(self.runObject.minPattern)
			StartGUI.agglomerative(StartGUI,self.runObject.dicSample[self.mainGUI.nameSample].dicSampleDiff,
				self.runObject.dicSample[self.mainGUI.nameSample], self.runObject,  self.runObject.minPattern)
			StartGUI.generateMatrixSimilarity(StartGUI,self.runObject.dicSample[self.mainGUI.nameSample].dicSampleDiff,
				self.runObject, 'Data/Similarity_Matrix_Sample', self.runObject.dicSample[self.mainGUI.nameSample].name)
		master.destroy()
		self.mainGUI.matrixWindow('Patterns Matrix', self.runObject.dicSample[self.mainGUI.nameSample].listGexelDiff,
			self.runObject.dicSample[self.mainGUI.nameSample].dicSampleDiff, 0, 0, 'umap', self.mainGUI.countDisplayStringDiff, self.mainGUI.nameSample, [])

	def fillListboxUmap(runObject, listBox):
		for cluster in runObject.dictClusterGexel.keys():
			listBox.insert(END, f"{cluster} | {len(runObject.dictClusterGexel[cluster])}")

	def setColorUmap(list_cluster):
		dictClusterColor = {}
		listColor = ['green', 'blue', 'red', 'gold', 'purple',
			'pink', 'orange', 'yellow', 'darkgreen', 'darkred', 
			'darkblue', 'limegreen', 'darkorange', 'deeppink', 'cyan']
		for i,cluster in enumerate(list_cluster):
			dictClusterColor[cluster] = listColor[i]
		return dictClusterColor

	def drawingUmap2(self):
		import umap
		import matplotlib.pyplot as plt
		import sklearn.cluster as cluster
		from matplotlib.backends.backend_tkagg import NavigationToolbar2Tk
		#script_dir = os.path.dirname(__file__)
		#abs_file_path = os.path.join(script_dir, "Data/BQ_matrix_Norm.tsv")
		_umap = umap.UMAP(n_components = 10, n_neighbors = self.n_neighbors, metric=self.metric, min_dist=self.min_dist)
		penguins = self.filterDiffGenes()
		#penguins = pd.read_csv(abs_file_path, sep='\t', index_col=[0]).astype(float)
		#transposed_csv = pd.DataFrame(data=penguins).T
		embedding =_umap.fit_transform(penguins)
		kmeans_labels = cluster.KMeans(n_clusters = self.n_clusters, n_init=10).fit_predict(penguins)
		fig = Figure(figsize = (8, 8), dpi = 100)
		plot1 = fig.add_subplot(111)
		sc = plot1.scatter(embedding[:,0], embedding[:,1], c = kmeans_labels)
		plot1.legend(*sc.legend_elements(), title='clusters')
		return fig

	
	def filterDiffGenes(self):
		from sklearn.preprocessing import StandardScaler
		penguins = self.runObject.norm[1].T
		print(penguins)
		dict_corr_DiffGenes = {}
		for gene in self.runObject.dicSample[self.nameSample].dicSampleDiff.values():
			for coordinate in gene.dic_coordinate_count.keys():
				if coordinate not in dict_corr_DiffGenes:
					dict_corr_DiffGenes[coordinate] = []
				if gene.dic_coordinate_count[coordinate] < self.thresholdUpDiff:
					dict_corr_DiffGenes[coordinate].append(gene.dic_coordinate_count[coordinate])
		#penguins = penguins.loc[:, features].values
